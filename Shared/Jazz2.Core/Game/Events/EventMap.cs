    ...
    public class EventMap : Disposable
    {
        ...
        public Vector2 GetSpawnPositionForMultiplayer()
        {
            if (spawnPositionsForMultiplayer.Count == 0) {
                ...
                var positions = spawnPositions[MathF.Rnd.OneOf(spawnPositions.Keys)];
                return positions[MathF.Rnd.Next(positions.Count)];
            }

            return spawnPositionsForMultiplayer[MathF.Rnd.Next(spawnPositionsForMultiplayer.Count)];
        }
#endif
