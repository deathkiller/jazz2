...
namespace Duality.Resources
{
	...
	public /*sealed*/ class Scene : Resource
	{
		...
		private RawList<Component> updatableComponents = new RawList<Component>(256);
		private RawList<UpdateEntry> updateMap = new RawList<UpdateEntry>();
		...
		internal void Update()
		{
			...
			if (this.lowFramerateMode) {
				this.UpdateComponents<ICmpFixedUpdatable>(cmp => cmp.OnFixedUpdate(Time.TimeMult));
			} else {
				this.fixedStepTimer += Time.DeltaTime;
				while (this.fixedStepTimer >= this.fixedStepPeriod) {
					this.UpdateComponents<ICmpFixedUpdatable>(cmp => cmp.OnFixedUpdate(1f));
		...
		public IEnumerable<Component> FindComponents(Type type)
		{
			...
			} else { // Select from a multitude of results
