    ...
    public class ChoiceControl : MenuControlBase
    {
        ...
        public override void OnDraw(Canvas canvas, ref Vector2 pos, bool focused)
        {
            ...
            for (int i = 0; i < choices.Length; i++) {
                ...
                if (selectedIndex == i) {
                    api.DrawMaterial("MenuGlow", x, pos.Y + 28f, Alignment.Center, ColorRgba.White.WithAlpha(0.2f), (choices[i].Length + 3) * 0.4f, 2.2f);

                    api.DrawStringShadow(ref charOffset, choices[i], x, pos.Y + 28f, Alignment.Center,
                        null, 0.9f, 0.4f, 0.55f, 0.55f, 8f, 0.9f);
                } else {
                    api.DrawString(ref charOffset, choices[i], x, pos.Y + 28f, Alignment.Center,
                        ColorRgba.TransparentBlack, 0.8f, charSpacing: 0.9f);
                }
            }

            api.DrawStringShadow(ref charOffset, "<", pos.X - (100f + 40f), pos.Y + 28f, Alignment.Center,
