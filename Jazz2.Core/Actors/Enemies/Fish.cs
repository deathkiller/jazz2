                ...
                for (int fx = 0; fx < currentAnimation.FrameDimensions.X; fx += debrisSize + 1) {
                    for (int fy = 0; fy < currentAnimation.FrameDimensions.Y; fy += debrisSize + 1) {
                        ...
                        api.TileMap.CreateDebris(new DestructibleDebris {
                            ...
                            Speed = new Vector2(((fx - currentAnimation.FrameDimensions.X / 2) + MathF.Rnd.NextFloat(-2f, 2f)) * (isFacingLeft ? -1f : 1f) * MathF.Rnd.NextFloat(1f, 3f) / currentAnimation.FrameDimensions.X,
                                 ((fy - currentAnimation.FrameDimensions.Y / 2) + MathF.Rnd.NextFloat(-2f, 2f)) * (isFacingLeft ? -1f : 1f) * MathF.Rnd.NextFloat(1f, 3f) / currentAnimation.FrameDimensions.Y),
