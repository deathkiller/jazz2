...
void main() {
    ...
    float red = texture2D(mainTex, vec2(uv.x - aberration, uv.y)).r;
    float blue = texture2D(mainTex, vec2(uv.x + aberration, uv.y)).b;
    main.rgb = mix(main.rgb, waterColor * (0.4 + 1.2 * vec3(red, main.g, blue)), vec3(isTexelBelow * 0.5));
    
    // Rays
    float noisePos = uvWorld.x * 8.0 + uvWorldCenter.y * 0.5 + (1.0 - gl_TexCoord[0].y - gl_TexCoord[0].x) * -5.0;
