...
void main() {
    ...
    // Noise
    vec3 noiseInput = vec3(24.0 * (gl_FragCoord.x + CameraPosition.x) / ViewSize.x, 24.0 * (gl_FragCoord.y + CameraPosition.y) / ViewSize.x, GameTime * 4.0);

    float noise = 1.0 - min(0.4,
        0.3 * snoise(noiseInput * 1.4) +
        0.5 * snoise(noiseInput) +
        0.3 * snoise(noiseInput * 0.6));

    float strength = noise * diffuseFactor * lightBlend(clamp(1.0 - ((dist - radiusNear) / (radiusFar - radiusNear)), 0.0, 1.0));
